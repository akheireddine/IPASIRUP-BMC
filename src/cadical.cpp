...
class App : public Handler, public Terminator {
  ...
  Solver * solver;                // Global CaDiCal solver.
  BMCOracle * oracleBMC;     // External Propagator component
...
int App::main (int argc, char ** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp (argv[i], "-h") ||
      ...
      witness = false;
    /* --------------------------------------------- */
    else if (with_bmc_oracle(argv[i])) {
      oracle_on = true;
    }
    /* --------------------------------------------- */
  ...
  if (incremental) {
    ...
    if(oracle_on)
      oracleBMC = new BMCOracle(solver, dimacs_name);
    res = solver->solve ();
    if(oracle_on){
